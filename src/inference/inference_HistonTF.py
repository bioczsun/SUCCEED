...
python src/inference/inference_HistonTF.py \
  ...
  --contig_bed /path/to/sequences.bed \
...
def parse_arguments():
    params = argparse.ArgumentParser(description='Inference using the HistonTF model')
    ...
    params.add_argument("--model", help="Model weights file path", required=True)
    params.add_argument("--seq_dir", help="DNA sequence directory", required=True)
    params.add_argument("--contig_bed", help="Inference contig bed file", required=True)
...
def main():
    ...
    hparams = vars(args)
    with open(f"{out_dir}/inference_config.yaml", 'w') as f:
    ...
    # Read contig_bed file
    if args.contig_bed.endswith('.bed'):
        contig_bed = open(args.contig_bed, "r").readlines()
        select_bed = []
        for i in contig_bed:
            i = i.split()
            cell_type = atac_dict[0].split('.')[0]  # Get cell type from the first ATAC filename
            select_bed.append(i[0:-1] + [cell_type])
    else:
        # Assume it's a npy file
        select_bed = np.load(args.contig_bed, allow_pickle=True)
    
    val_dataset = HistonTFDataset(
                    contig_bed=select_bed,
                    seq_dir=args.seq_dir,
                    feature_dir=None,  # No label data needed in inference mode
    ...
    evaluate(model, valid_par, device, model_save_path)
    print(f"Inference complete, results saved to {model_save_path}")


def evaluate(model, valid_data, device, out_dir):
