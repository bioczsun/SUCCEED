...
class GenomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, 
                       ...
                       use_aug = True,
                       sequence_embedding_h5 = None,
                       sequence_embedding_key = 'embeddings',
                       intervals_bed = None):
        ...
        self.sequence_embedding_provider = None
        self.seq_input_channels = 4
        if sequence_embedding_h5 is not None:
            if intervals_bed is None:
                raise ValueError('intervals_bed must be provided when using precomputed sequence embeddings.')
            bed_path = os.path.abspath(os.path.expanduser(intervals_bed))
            h5_path = os.path.abspath(os.path.expanduser(sequence_embedding_h5))
            print(f'Using precomputed sequence embeddings from {h5_path}\nInterval BED: {bed_path}')
            self.sequence_embedding_provider = data_feature.H5IntervalEmbeddingProvider(
                bed_path, h5_path, dataset_key=sequence_embedding_key
            )
            self.seq_input_channels = self.sequence_embedding_provider.embedding_dim
    ...
    def load_chrs(self, chr_names, genomic_features):
        ...
        for chr_name in chr_names:
            ...
            chr_data_dict[chr_name] = ChromosomeDataset(self.data_root, chr_name, omit_regions, genomic_features, self.use_aug, precomputed_seq_embeddings=self.sequence_embedding_provider)
