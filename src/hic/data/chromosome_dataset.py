...
import hic.data.data_feature as data_feature
...
class ChromosomeDataset(Dataset):
    ...
    '''
    def __init__(self, celltype_root, chr_name, omit_regions, feature_list, use_aug = True, precomputed_seq_embeddings = None):
        self.precomputed_seq_embeddings = precomputed_seq_embeddings
        self.use_precomputed_seq_embeddings = self.precomputed_seq_embeddings is not None
        self.use_aug = use_aug and not self.use_precomputed_seq_embeddings
        ...
        self.seq = None
        if not self.use_precomputed_seq_embeddings:
        ...
        if self.use_precomputed_seq_embeddings:
            intervals, indices = self.precomputed_seq_embeddings.get_chr_intervals(chr_name)
            if len(intervals) == 0:
                raise ValueError(f'No intervals found for {chr_name} in BED file.')
            self.all_intervals = intervals
            self.intervals, self.interval_indices = self.filter(self.all_intervals, omit_regions, indices)
            self.intervals = np.array(self.intervals)
            self.interval_indices = np.array(self.interval_indices)
        else:
            ...
            self.intervals = np.array(self.all_intervals)

    def __getitem__(self, idx):
        start, end = self.intervals[idx]
        interval_idx = self.interval_indices[idx] if self.interval_indices is not None else None
        if self.use_precomputed_seq_embeddings:
            ...
            # Shift Augmentations
            if self.use_aug:
                target_start, target_end = self.shift_aug(target_size, start, end)
            else:
                target_start, target_end = self.shift_fix(target_size, start, end)
        seq, features, mat = self.get_data_at_interval(target_start, target_end, interval_idx)
    ...
    def get_data_at_interval(self, start, end, interval_idx = None):
        ...
        if self.use_precomputed_seq_embeddings:
            if interval_idx is None:
                raise ValueError('Interval index required when using precomputed embeddings.')
            seq = self.precomputed_seq_embeddings.get_embedding(interval_idx)
        else:
            seq = self.seq.get(start, end).astype(np.float32)
        # Features processing
        features = [item.get(self.chr_name, start, end).astype(np.float32) for item in self.genomic_features]
    ...
    def filter(self, intervals, omit_regions, indices = None):
        valid_intervals = []
        valid_indices = [] if indices is not None else None
        ...
        if valid_indices is not None:
            return valid_intervals, valid_indices
