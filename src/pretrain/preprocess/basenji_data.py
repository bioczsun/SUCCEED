...
def main():
  ...
  if not options.restart:
    ...
    # Only chromosomes 1-22 and X and Y remain
    valid_chromosomes = [f'chr{i}' for i in range(1, 23)] + ['chrX', 'chrY']
    contigs = [ctg for ctg in contigs if ctg.chr in valid_chromosomes]
    # limit to a BED file
  ...
  for ti in range(targets_df.shape[0]):
    ...
    if options.restart and os.path.isfile(seqs_cov_file):
      ...
      cmd = 'python src/preprocess/basenji_data_read.py'
...
def divide_contigs_chr(contigs, test_chrs, valid_chrs):
  ...
  # Keep only chromosomes 1-22 plus X and Y
  # print(contigs)
  valid_chromosomes = [f'chr{i}' for i in range(1, 23)] + ['chrX', 'chrY']
  contigs = [ctg for ctg in contigs if ctg.chr in valid_chromosomes]
