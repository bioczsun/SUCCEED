...
def main():
  ...
  seqs_cov_open.close()
  # Convert the H5 targets to a NumPy file for faster loading
  np.save(f'{seqs_cov_file}.npy', targets)
  # Remove the temporary H5 file
  os.remove(seqs_cov_file)
...
class CovFace:
  ...
  def read(self, chrm, start, end):
    if self.bigwig:
        # Debugging: Print the chromosome, start, and end
        # print(f"Reading {chrm}:{start}-{end}")
                
        # Check if the interval is valid
        if start < 0 or end < 0 or start >= end:
            print(f'>{self.cov_file}')
            raise RuntimeError(f"Invalid interval bounds: {chrm}:{start}-{end}")
        
        # Check if the chromosome exists in the bigwig file
        if chrm not in self.cov_open.chroms():
            raise RuntimeError(f"Chromosome {chrm} not found in the bigwig file.")
        
        # Check if the interval is within the chromosome bounds
        chrom_length = self.cov_open.chroms(chrm)
        if end > chrom_length:
            print(f'>{self.cov_file}')
            raise RuntimeError(f"Interval {chrm}:{start}-{end} exceeds chromosome length {chrom_length}.")
