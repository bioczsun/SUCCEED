...
import sys
from corigami.model.Enformer import Enformer
...
config_args.max_seq_len = 256
config_args.target_seq_len = 256
model_epi = Enformer(config_args, return_emb=True)

# 从main.py导入args
from corigami.training.main import args
if args.use_pth:
...
class EncoderSplit(Encoder):
    def __init__(self, num_epi, output_size = 256, filter_size = 5, num_blocks = 12):
        ...
        if self.succeed:
            ...
            self.conv_end = nn.Conv1d(384, output_size, 1)
        else:
            self.conv_end = nn.Conv1d(256, output_size, 1)
