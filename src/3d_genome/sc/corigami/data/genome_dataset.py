...
class GenomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, 
        ...
        self.chr_data, self.lengths = self.load_chrs(self.chr_names, self.cell_type)
    ...
    def __getitem__(self, idx):
        ...
        i,chr_name, chr_idx = self.get_chr_idx(idx)
        chr_name = f"{chr_name}_{i}"
    ...
    def load_chrs(self, chr_names, cell_type):
        '''
        ...
        for chr_name in chr_names:
            for i in cell_type:
                genomic_features = self.load_features(f'{self.data_root}/../{i}/genomic_features', self.feat_dicts)
                omit_regions = self.centrotelo_dict[chr_name]
                chr_data_dict[f"{chr_name}_{i}"] = ChromosomeDataset(self.data_root,i, chr_name, omit_regions, genomic_features, self.use_aug)
                lengths.append(len(chr_data_dict[f"{chr_name}_{i}"]))
    ...
    def get_chr_idx(self, idx):
        ...
        for i, chr_range in enumerate(self.ranges):
            start, end = chr_range
            if start <= idx <= end:
                return self.cell_type[i % len(self.cell_type)],self.chr_names[i // len(self.cell_type)], idx - start
