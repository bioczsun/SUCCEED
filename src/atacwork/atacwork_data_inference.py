...
parser.add_argument("--limit_chrom", type=str, help="limit to a chromosome")
...
chromsome_limit = params.limit_chrom
...
def main():
    ...
    if params.restart:
        ...
        train_contigs = rejoin_large_contigs(contigs)
        ...
        all_mseqs = train_mseqs

        if chromsome_limit and chromsome_limit != "all":
          tmp_mseqs = []
          for mseq in all_mseqs:
              if mseq.chr == chromsome_limit:
                  tmp_mseqs.append(mseq)
          all_mseqs = tmp_mseqs
...
def get_seq_targets(contig, sample_file):
...
def map_seq_to_targets(contig,fasta,sample_file):
    # Fetch and encode sequence
    seq = fasta.fetch(contig.chr, int(contig.start), int(contig.end))
    sample_coverage = get_seq_targets(contig, sample_file)
    one_hot_seq_array = dna_1hot(seq)

    return one_hot_seq_array, sample_coverage
...
def split_sequences(results,output_dir):
    train_seqs, train_sample = [], []
    for one_hot_seq, sample_coverage, in results:
        train_seqs.append(one_hot_seq)
        train_sample.append(sample_coverage)
    # 保存训练集
    with h5py.File("%s/inference_data_%s.h5"%(output_dir,cell_name), "w") as train_h5:
...
if __name__ == '__main__':
    ...
    def map_seq_to_targets_partial(contig):
        return map_seq_to_targets(contig, fasta_open, sample_file)
