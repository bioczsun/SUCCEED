...
def get_seq_targets(contig, sample_file, full_file,chromosome_trees):
    ...
    sample_coverage_tmp = sample_coverage.reshape(-1,128).max(axis=1)
    ...
    ## 如果sample_coverage_tmp中，0的比例大于90%，则返回None
    if np.sum(sample_coverage_tmp == 0.0) / len(sample_coverage_tmp) > 0.995:
        return 0,0,0,0

    return 1,sample_coverage, full_coverage, label_ls

def map_seq_to_targets(contig,fasta,sample_file,full_file):
    # Fetch and encode sequence
    bi,sample_coverage, full_coverage,peak_label = get_seq_targets(contig, sample_file, full_file,chromosome_trees)
    if bi != 0:
